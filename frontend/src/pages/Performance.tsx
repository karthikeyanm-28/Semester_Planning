  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Track your academic performance and insights</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KpiCard title="Semester Completion" value={`${stats.completion}%`} icon={TrendingUp} variant={stats.completion > 60 ? 'success' : 'warning'} trend={`${stats.completion > 60 ? '📈' : '⚠️'} progress`} />
        <KpiCard title="Deadline Adherence" value={`${stats.adherence}%`} icon={Target} variant={stats.adherence > 70 ? 'success' : 'danger'} trend={`${stats.adherence > 70 ? '✓' : '⚠'} on track`} />
        <KpiCard title="Productivity Score" value={stats.weeklyScore} icon={Zap} variant={stats.weeklyScore > 50 ? 'success' : 'warning'} trend={`${stats.weeklyScore > 50 ? 'Strong' : 'Boost needed'}`} />
      </div>
